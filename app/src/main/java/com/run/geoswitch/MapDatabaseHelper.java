...
public class MapDatabaseHelper extends SQLiteOpenHelper {
    ...
    public static final String TABLE_NAME = "mymaplist_data";
    ...
    public void onCreate(SQLiteDatabase db) {
        String createTable = "CREATE TABLE " + TABLE_NAME + " (ID INTEGER PRIMARY KEY AUTOINCREMENT, " +
                " ITEM1 TEXT , ITEM2 INTEGER , ITEM3 TEXT, ITEM4 INTEGER)";
    ...
    public void addData(String item1, int item2, String item3, int item4) {
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues run = new ContentValues();
        run.put(COL2, item1);//name
        run.put(COL3, item2);//token
        run.put(COL4, item3);//latlng
        run.put(COL5, item4);//radius
